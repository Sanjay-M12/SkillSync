...
export const plannerApi = {
  getDailyPlan: async (targetMinutes?: number): Promise<DailyLearningPlan> => {
    const res = await api.get<ApiResponse<DailyLearningPlan>>(
      `/planner/today${targetMinutes ? `?targetMinutes=${targetMinutes}` : ""}`
    )
    return res.data!
  },
}
